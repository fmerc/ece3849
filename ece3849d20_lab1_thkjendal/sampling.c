...
volatile uint16_t sample[ADC_TRIGGER_SIZE];
...
int RisingTrigger(void) {   // search for rising edge trigger
    ...
    case 0:
        ...
        break;
    case 1:
        x_stop = x + ADC_BUFFER_SIZE/2;
        for (x; x < x_stop; x++) {
            if (    gADCBuffer[ADC_BUFFER_WRAP(x+1)] <= ADC_OFFSET &&
                    gADCBuffer[ADC_BUFFER_WRAP(x)] > ADC_OFFSET)
                break;
        }
    }
    // Step 3
